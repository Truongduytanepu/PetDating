...
class ProfileTableViewCell: UITableViewCell, UIPickerViewDelegate,UIPickerViewDataSource {
    ...
    override func awakeFromNib() {
        ...
        // Lấy giá trị slider từ UserDefaults
        if let lowerValue = UserDefaults.standard.value(forKey: "lowerSliderValue") as? Double,
           let upperValue = UserDefaults.standard.value(forKey: "upperSliderValue") as? Double {
            rangeSlider.lowerValue = lowerValue
            rangeSlider.upperValue = upperValue
            updateResultLabel(lowerValue: lowerValue, upperValue: upperValue)
        }
        
        // Lấy giá trị giới tính từ UserDefaults (nếu có)
        if let selectedGender = UserDefaults.standard.value(forKey: "selectedGender") as? String {
            showGender.setTitle(selectedGender, for: .normal)
        }
    ...
    @objc func sliderValueChanged(_ slider: RangeSlider) {
        ...
        // Lưu giá trị của slider vào UserDefaults
        UserDefaults.standard.set(lowerValue, forKey: "lowerSliderValue")
        UserDefaults.standard.set(upperValue, forKey: "upperSliderValue")
    }
    ...
    func showGenderPicker() {
        ActionSheetStringPicker.show(withTitle: "Select Gender", rows: gender, initialSelection: 0, doneBlock: { [weak self] picker, selectedIndex, selectedValue in
            ...
            // Lưu giá trị giới tính vào UserDefaults
